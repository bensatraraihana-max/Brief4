...
function afficherErreur(champId, messageErreur) {
  ...
  // Ajouter le message juste après le champ
  champ.insertAdjacentElement("afterend", messageErreurElement);

  // Mettre la bordure rouge
...
function validerFormulaireContact() {
  ...
  function validation(e) {
    ...
    if (prenom === "" || !regexNom.test(prenom)) {
      afficherErreur("firstName", "Le prénom est incorrect. Resaisissez-le.");
      formulaireValide = false;
    }

    // Validation du nom
    if (nom === "" || !regexNom.test(nom)) {
      afficherErreur("lastName", "Le nom est incorrect. Resaisissez-le.");
      formulaireValide = false;
    }
...
function creerBarreRecherche() {
  ...
  inputRecherche.placeholder =
    "Rechercher une mission (nom, agence, objectif...)";
...
function filtrerMissions(termeRecherche) {
  ...
  toutesLesMissions.forEach(function (mission) {
    ...
    const texte =
      mission.querySelector(".mission-text p")?.textContent.toLowerCase() || "";

    // Vérifier la recherche
    const correspondRecherche =
      titre.includes(termeRecherche) || texte.includes(termeRecherche);

    // Vérifier le filtre agence (à implémenter avec des data attributes dans le HTML)
    const correspondAgence =
      agenceSelectionnee === "" ||
      texte.includes(agenceSelectionnee.toLowerCase());
...
function afficherFavoris() {
  ...
  if (favoris.length === 0) {
    listeFavoris.innerHTML =
      '<p style="color: #666;">Aucune mission favorite pour le moment</p>';
    return;
  }
...
function afficherFormulaireAjout() {
  ...
  document
    .getElementById("form-ajout-mission")
